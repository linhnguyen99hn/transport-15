...
class Baocaokehoach(models.TransientModel):
    _name = 'bao.cao.ke.hoach'
    ...
    def get_report(self):
        ...
        data = {
            ...
            'ids': self.ids,
            'form': {
                'date_start': date_start, 'date_end': date_end, 'domain': domain, 'donhang': self.donhang, 'may': self.maychay.mamay,
                'sanpham': self.sanpham, 'madonhang': self.donhang.madonhang, 'tensanpham': self.sanpham.tensanpham,
                'lenhsx': self.lenhsx.lenhsx
            },
        }

        action = self.env.ref('mrp_fm.action_bao_cao_ke_hoach').report_action(self, data=data)
        return action

class Baocaodoanhso(models.AbstractModel):
    _name = 'report.mrp_fm.bao_cao_ke_hoach_document'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        docs = self.env['kehoach.sanxuat'].search(domain, order="ngay asc")
